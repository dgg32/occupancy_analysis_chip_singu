...
class Int2npy(object):
    ...
    def __init__(self, data_dp, fov, start_cycle, cycle_range=10, read_len=20, output_dp='', int_type='FInt',
    ...
    def run(self):
        ...
        for i, cycle in enumerate(cycles):
            #int_bin_fp = os.path.join(self.data_dp, self.int_type, 'S%03d' % cycle, '%s.int' % self.fov)
            int_bin_fp = os.path.join(self.data_dp, self.int_type, '%s.FInt' % self.fov)
