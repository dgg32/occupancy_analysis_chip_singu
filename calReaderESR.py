...
class Cal(object):
    ...
    def _getBaseQual(self, idPrefix="", cycles=False):
        ...
        # qualChar = allQualArr.view("S1").reshape(-1, len(self.cycleSet))
        qualChar = allQualArr.view("S1").T
        return idPrefix, baseChar, qualChar
    ...
    def _dumpAsFqSE(self, fp, idPrefix="", cycles=False, filter_ESR=False):
        idPrefix, baseChar, qualChar = self._getBaseQual(idPrefix=idPrefix, cycles=cycles)
        ...
        if filter_ESR:
            counter = 0
            for i in range(len(baseChar)):
                d = np.array([ord(ch) - 33 for ch in qualChar[i][:cyclesNum]])
                if len(np.where(d < qualThres)[0]) <= qualCountThres:
                    counter += 1
                    fp.write("@%s%06d\n" % (idPrefix, i))
                    fp.write(baseChar[i].tostring().decode("utf8"))
                    fp.write("\n+\n")
            ...
            for i in range(len(baseChar)):
                ...
                fp.write(qualChar[i].tostring().decode("utf8"))
                fp.write("\n")
        return (100. * counter / len(baseChar))

    def _dumpAsFqPE(self, fps, idPrefixes=None, cycles=False, filter_ESR=False):
        id1, baseChar1, qualChar1 = self._getBaseQual(idPrefix=idPrefixes[0], cycles=cycles[0])
        ...
        filtered = []
        if filter_ESR:
            for n, idPrefix, baseChar, qualChar in zip(range(len(fps)), [id1, id2], [baseChar1, baseChar2],
                                                       [qualChar1, qualChar2]):
                strand = n + 1
                qq = qualThres[strand]
                for i, qual in enumerate(qualChar[:, :cyclesNum]):
                    d = np.array([ord(ch) - 33 for ch in qual])
                    if len(np.where(d < qq)[0]) > qualCountThres:
                        dnbs[strand].append(i)
            # filter out if either read below thres
            filtered = np.setdiff1d(range(dnb_num), list(set(dnbs[1] + dnbs[2])))
            esr_arr = np.zeros(dnb_num, dtype=bool)
        ...
        for fp, idPrefix, baseChar, qualChar in zip(fps, [id1, id2], [baseChar1, baseChar2], [qualChar1, qualChar2]):
            ...
            for i in rg:
                ...
                fp.write(qualChar[i].tostring().decode("utf8"))
                fp.write("\n")
        return (100. * len(filtered) / dnb_num)

    def writefq(self, filename, idPrefix=None, cycles=None, filter_ESR=False):
        ...
        if filename.endswith('.gz'):
            ...
            if type(cycles) is list:
                if type(cycles[0]) is int:
                    ...
                    #     split_name = filename.replace('.fq.gz', suffix)
                    #     with gzip.open(split_name, 'wb') as fp:
                    #         self._dumpAsFq(fp, idPrefix=idPrefix, cycles=read_bounds)
                    #     tag += 1
            elif type(cycles) == int:
                with gzip.open(filename, 'wb') as fp:
                    self._dumpAsFqSE(fp, idPrefix=idPrefix, cycles=cycles, filter_ESR=filter_ESR)
            else:
                with gzip.open(filename, 'wb') as fp:
                    self._dumpAsFqSE(fp, idPrefix=idPrefix, filter_ESR=filter_ESR)
        else:
